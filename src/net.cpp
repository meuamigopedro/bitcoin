    ...
    while (!interruptNet)
    {
        ...
        while (!interruptNet)
        {
            ...
            if (fFeeler) {
                ...
                std::tie(addr, nLastTry) = addrman.SelectTriedCollision();

                if (!addr.IsValid()) {
                    // No tried table collisions. Select a new table address
                    // for our feeler.
                    std::tie(addr, nLastTry) = addrman.Select(true);
                ...
                // Not a feeler
                std::tie(addr, nLastTry) = addrman.Select();
            }
            ...
            if (nANow - nLastTry < 600 && nTries < 30)
                continue;
