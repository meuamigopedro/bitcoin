...
BOOST_AUTO_TEST_CASE(outbound_slow_chain_eviction)
{
    ...
    auto connman = std::make_unique<CConnman>(0x1337, 0x1337, *m_node.addrman, *banman);
    auto peerLogic = PeerManager::make(chainparams, *connman, *m_node.addrman, *banman,
...
BOOST_AUTO_TEST_CASE(stale_tip_peer_management)
{
    const CChainParams& chainparams = Params();
    auto banman = std::make_unique<BanMan>(m_args.GetDataDirBase() / "banlist", nullptr, DEFAULT_MISBEHAVING_BANTIME);
    auto connman = std::make_unique<ConnmanTestMsg>(0x1337, 0x1337, *m_node.addrman, *banman);
    auto peerLogic = PeerManager::make(chainparams, *connman, *m_node.addrman, *banman,
                                       *m_node.chainman, *m_node.mempool, false);
