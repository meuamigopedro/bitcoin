...
{
    auto addrman = std::make_unique<CAddrMan>(std::vector<bool>(), /* makeDeterministic */ true, /* consistency_check_ratio */ 100);
    ...
    addrman = std::make_unique<CAddrMan>(std::vector<bool>(), /* makeDeterministic */ true, /* consistency_check_ratio */ 100);
...
BOOST_AUTO_TEST_CASE(addrman_serialization)
{
    ...
    auto addrman_asmap1 = std::make_unique<CAddrMan>(asmap1, true, 100);
    auto addrman_asmap1_dup = std::make_unique<CAddrMan>(asmap1, true, 100);
    auto addrman_noasmap = std::make_unique<CAddrMan>(std::vector<bool>(), true, 100);
    ...
    AddressPosition addr_pos1 = addrman_asmap1->FindEntry(addr);
    AddressPosition addr_pos2 = addrman_asmap1_dup->FindEntry(addr);
    ...
    AddressPosition addr_pos3 = addrman_noasmap->FindEntry(addr);
    ...
    addrman_asmap1 = std::make_unique<CAddrMan>(asmap1, true, 100);
    addrman_noasmap = std::make_unique<CAddrMan>(std::vector<bool>(), true, 100);
    addrman_noasmap->Add({addr}, default_source);
    stream << *addrman_noasmap;
    stream >> *addrman_asmap1;

    AddressPosition addr_pos4 = addrman_asmap1->FindEntry(addr);
    ...
    AddressPosition addr_pos5 = addrman_noasmap->FindEntry(addr1);
    AddressPosition addr_pos6 = addrman_noasmap->FindEntry(addr2);
    BOOST_CHECK(addr_pos5.bucket != addr_pos6.bucket);
    stream << *addrman_noasmap;
    stream >> *addrman_asmap1;
    AddressPosition addr_pos7 = addrman_asmap1->FindEntry(addr1);
    AddressPosition addr_pos8 = addrman_asmap1->FindEntry(addr2);
...
BOOST_AUTO_TEST_CASE(addrman_evictionworks)
{
    ...
    auto info = addrman->SelectTriedCollision();
    BOOST_CHECK_EQUAL(info.ToString(), "250.1.1.19:0");

    // Ensure test of address fails, so that it is evicted.
    // Set last good connection in the deep past.
    addrman->Good(info, /*nLastSuccess */ 1);
    addrman->Attempt(info, /* count_failure */ false, /* nLastTry */ GetAdjustedTime() - 61);
