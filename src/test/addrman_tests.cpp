...
BOOST_AUTO_TEST_CASE(addrman_simple)
{
    ...
    auto addr_ret1 = std::get<0>(addrman->Select());
...
BOOST_AUTO_TEST_CASE(addrman_select)
{
    ...
    bool newOnly = true;
    auto addr_ret1 = std::get<0>(addrman.Select(newOnly));
    BOOST_CHECK_EQUAL(addr_ret1.ToString(), "250.1.1.1:8333");

    // Test: move addr to tried, select from new expected nothing returned.
    addrman.Good(CAddress(addr1, NODE_NONE));
    BOOST_CHECK_EQUAL(addrman.size(), 1U);
    auto addr_ret2 = std::get<0>(addrman.Select(newOnly));
    BOOST_CHECK_EQUAL(addr_ret2.ToString(), "[::]:0");

    auto addr_ret3 = std::get<0>(addrman.Select());
    BOOST_CHECK_EQUAL(addr_ret3.ToString(), "250.1.1.1:8333");
    ...
    for (int i = 0; i < 20; ++i) {
        ports.insert(std::get<0>(addrman.Select()).GetPort());
    }
...
BOOST_AUTO_TEST_CASE(addrman_selecttriedcollision)
{
    ...
    BOOST_CHECK(std::get<0>(addrman.SelectTriedCollision()).ToString() == "[::]:0");
...
BOOST_AUTO_TEST_CASE(addrman_noevict)
{
    ...
    BOOST_CHECK_EQUAL(std::get<0>(addrman.SelectTriedCollision()).ToString(), "250.1.1.19:0");
    ...
    BOOST_CHECK_EQUAL(std::get<0>(addrman.SelectTriedCollision()).ToString(), "250.1.1.10:0");
...
BOOST_AUTO_TEST_CASE(addrman_evictionworks)
{
    ...
    auto info = std::get<0>(addrman.SelectTriedCollision());
    ...
    BOOST_CHECK_EQUAL(std::get<0>(addrman.SelectTriedCollision()).ToString(), "250.1.1.36:0");
